...
export const MiniOfferModal = ({ onClose }) => {
  const formConfig = courseForms['Mini offer - product ideas + headline and benefits'] || [];
  ...
  const handleSubmit = async (e) => {
    ...
    const url = `${API_BASE_URL}/module4/mini-offer`;
    const payload = {
      mini_offer_type: formData.mini_offer_type || "",
      mini_offer_title: formData.mini_offer_title || "",
      niche: formData.niche || "",
      key_promise: formData.key_promise || "",
      content_format: formData.content_format || "",
      target_group: formData.target_group || "",
      pain_point: formData.pain_point || "",
      key_elements: formData.key_elements || "",
  ...
  return (
    <div className="fixed bottom-20 right-6 z-50 w-[400px] h-[450px] bg-white rounded-xl shadow-2xl flex flex-col border border-gray-200">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 bg-[#5035f6] rounded-t-xl">
        <h2 className="text-[15px] font-bold text-white">Mini Offer - Product Ideas + Headline and Benefits</h2>
        <button
