...
export const ScriptContentRollModal = ({ onClose }) => {
  const formConfig = courseForms['Script for the content roll'] || [];
  ...
  const handleSubmit = async (e) => {
    ...
    const url = `${API_BASE_URL}/module4/content-roll-script`;
    const payload = {
      niche: formData.niche || "",
      offer_type: formData.offer_type || "",
      offer_title: formData.offer_title || "",
      offer_promise: formData.offer_promise || "",
      offer_link: formData.offer_link || "",
      audience: formData.audience || "",
      pain_point: formData.pain_point || "",
      goal: formData.goal || "",
      awareness_level: formData.awareness_level || "",
      reel_topic: formData.reel_topic || "",
      reel_tips: formData.reel_tips || "",
      expertise: formData.expertise || "",
      tone: formData.tone || "",
      sales_style: formData.sales_style || ""
    };
    try {
  ...
  return (
    <div className="fixed bottom-20 right-6 z-50 w-[400px] h-[450px] bg-white rounded-xl shadow-2xl flex flex-col border border-gray-200">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 bg-[#5035f6] rounded-t-xl">
        <h2 className="text-[15px] font-bold text-white">Script for the Content Roll</h2>
        <button
          ...
          <div ref={responseRef} className="mt-4 p-3 bg-gray-100 rounded text-sm text-gray-800 border border-gray-200">
            {apiResponse.result ? (
              <>
                ...
                <div className="whitespace-pre-wrap">{apiResponse.result}</div>
              </>
            ) : apiResponse.error ? (
