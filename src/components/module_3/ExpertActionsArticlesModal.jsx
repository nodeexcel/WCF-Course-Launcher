...
export const ExpertActionsArticlesModal = ({ onClose }) => {
  const formConfig = courseForms['Expert Actions - Articles'] || [];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        niche: formData.brandNicheTheme || '',
        target_group: formData.targetGroup || '',
        problems: formData.problemsSolved || '',
        outcomes: formData.effectsAchieved || ''
      };
      const response = await fetch(`${API_BASE_URL}/module3/expert-actions`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="fixed bottom-20 right-6 z-50 w-[400px] h-[450px] bg-white rounded-xl shadow-2xl flex flex-col border border-gray-200">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 bg-[#5035f6] rounded-t-xl">
        <h2 className="text-[15px] font-bold text-white">Expert Actions - Articles</h2>
        <button
