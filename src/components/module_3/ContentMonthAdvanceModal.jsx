...
export const ContentMonthAdvanceModal = ({ onClose }) => {
  const formConfig = courseForms['Creating Content a Month in Advance'] || [];
  ...
  useEffect(() => {
    // Autofill 'mainTopicNiche' from localStorage if available (from Choosing a Niche or Market Research)
    const selectedNiche = localStorage.getItem('selected_Choosing a Niche') || localStorage.getItem('selected_Market Research') || '';
    if (selectedNiche) {
      setFormData(prev => ({ ...prev, mainTopicNiche: selectedNiche }));
    }
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/module3/creating-content`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="fixed bottom-20 right-6 z-50 w-[400px] h-[450px] bg-white rounded-xl shadow-2xl flex flex-col border border-gray-200">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 bg-[#5035f6] rounded-t-xl">
        <h2 className="text-[15px] font-bold text-white">Creating Content a Month in Advance</h2>
        <button
