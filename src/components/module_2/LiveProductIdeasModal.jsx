...
export const LiveProductIdeasModal = ({ onClose }) => {
  const formConfig = courseForms['Live Product iDEAS'] || [];
  ...
  const handleSubmit = async (e) => {
    ...
    const url = `${API_BASE_URL}/module2/live-product-ideas`;
    const payload = {
      niche: formData.niche,
      problems_list: formData.problems_list,
      target_group_description: formData.target_group_description,
    };
    try {
  ...
  return (
    <div className="fixed bottom-20 right-6 z-50 w-[400px] h-[450px] bg-white rounded-xl shadow-2xl flex flex-col border border-gray-200">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 bg-[#5035f6] rounded-t-xl">
        <h2 className="text-[15px] font-bold text-white">Live Product iDEAS</h2>
        <button
